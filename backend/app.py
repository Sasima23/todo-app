...
def row_to_task(row):
    due = row["due_date"]
    overdue = bool(due) and not bool(row["done"]) and due < date.today().isoformat()
    return {
        "id": row["id"],
        "title": row["title"],
        "done": bool(row["done"]),
...
def create_task():
    ...
    db = get_db()
    cur = db.execute(
        """INSERT INTO tasks (title, done, priority, category, due_date, created_at)
           VALUES (?, 0, ?, ?, ?, ?)""",
        (title, priority, category, due_date, datetime.now(timezone.utc).isoformat()),
    )
    db.commit()
...
def update_task(task_id):
    ...
    db.execute(
        """UPDATE tasks SET title = ?, done = ?, priority = ?, category = ?, due_date = ?
           WHERE id = ?""",
        (title, 1 if done else 0, priority, category, due_date, task_id),
    )
    db.commit()
    row = db.execute("SELECT * FROM tasks WHERE id = ?", (task_id,)).fetchone()
